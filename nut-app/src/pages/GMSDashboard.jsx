...
const C = {
  bg: '#faf8f5', card: '#ffffff', primary: '#7c3aed', primaryLight: '#ede9fe',
...
export default function GMSDashboard() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'system-ui, sans-serif' }}>
      <header style={{ background: `linear-gradient(135deg, ${C.primary}, #5b21b6)`, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div>
          <div style={{ color: '#fff', fontWeight: 800, fontSize: 20 }}>🏪 {profile.name}</div>
          <div style={{ color: '#ddd6fe', fontSize: 13 }}>Grande & Moyenne Surface</div>
        </div>
      ...
      <div style={{ maxWidth: 600, margin: '0 auto', padding: 20 }}>
        ...
        <div style={{ ...s.card, textAlign: 'center', background: C.primaryLight }}>
          <div style={{ fontSize: 14, color: C.primary, fontWeight: 600 }}>Caisses en réception</div>
          <div style={{ fontSize: 42, fontWeight: 800, color: C.primary }}>{crates.length}</div>
        </div>

        {/* Liste caisses */}
        <div style={s.card}>
          <h3 style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 700 }}>📦 Caisses reçues</h3>
          {loading ? (
            <div style={{ textAlign: 'center', padding: 20, color: C.muted }}>⏳ Chargement...</div>
          ) : crates.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 20, color: C.muted }}>Aucune caisse en stock</div>
          ) : (
        ...
        {/* Historique */}
        <div style={s.card}>
          <h3 style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 700 }}>📋 Historique des mouvements</h3>
          {movements.length === 0 ? (
